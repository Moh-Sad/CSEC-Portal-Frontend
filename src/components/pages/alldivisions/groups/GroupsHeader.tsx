...
export function GroupsHeader({ className, onGroupAdded }: TableFilterProps) {
  ...
  useEffect(() => {
    const checkUserInDivision = async () => {
      ...
      try {
        ...
        const isMember = response.data.some((group: any) => group.members.includes(userId))
  ...
  return (
    <div className={cn("flex justify-end items-center p-2 sm:p-5", className)}>
      <div className="hidden sm:flex justify-center gap-1 p-2 border-1 border-gray-300 rounded-[8px] h-12 items-center focus:outline-blue-600 focus:border-blue-600">
        <LuSearch size={23} />
        <div>
          <Input
            type="text"
